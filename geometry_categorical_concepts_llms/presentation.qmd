...
- Inner product $\langle u,v \rangle = u^T G v$.
  ...
  - Then, we can use $\langle u,v \rangle = u^T X X^T v$, meaning that we are mapping the vectors to a new space, and use the Euclidian inner product in the new space.
...
<!-- ## Degrees of Freedom in Choosing  $M$

- Causal orthogonality imposes $\frac{d(d-1)}{2}$ constraints.
- Identifying $M$ has $\frac{d(d-1)}{2} + d$ degrees of freedom.
- So, there are **$d$** degrees of freedom left — one for each diagonal entry in $D$.
- Each choice of diagonal $D$ defines a valid causal inner product. -->
...
- No unique choice of causal inner product.
- **Most inner products can be ruled out.**
- E.g., **Euclidean inner product** is **not causal** if $M = I_d$ because it does not satisfy:
$$M^{-1} = GG^\top \quad \text{and} \quad G^\top \text{Cov}(\gamma)^{-1} G = D$$
  for any $D$. -->
